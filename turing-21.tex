...
\author[Whiteson \& Rashid (Oxford)]{Shimon Whiteson \& Tabish Rashid \\ Dept.\ of Computer Science \\ University of Oxford \\ \ \\ joint work with Jakob Foerster, Gregory Farquhar, Anuj Mahajan,\\ Mikayel Samvelyan, Christian Schroeder de Witt,
...
\frame{
	...
	\begin{center}
		\includegraphics[width=0.7\textwidth]{Shimon}
		\begin{itemize}
			\item How important is this in general?
		\end{itemize}
	\end{center}
}
...
\frame{
	...
	\begin{itemize}
		\item We know QMIX cannot represent \textbf{all} possible joint-action $Q$-values.\\
		\item Can have catastrophic consequences in the following situation:\\
		(originally from [Son. et al 2019])
	\end{itemize}
	\begin{center}
		\begin{tabular}{ | c | c | c |}
			\hline
...
\frame{
	...
	\begin{itemize}
		\item These limitations are a problem because they are a \textbf{fundamental} part of QMIX.\\
		\item We have deliberately chosen to factorise our joint-action $Q$-value in this manner.\\
		\item Cannot get around these limitations by using bigger networks, more training, more compute, etc.\\
	\end{itemize}
	\begin{center}
...
\frame{
	...
	\begin{itemize}
		\item Let's consider an \textit{idealised} version of QMIX.
		\item We can do this through an \textit{operator}, that represents QMIX:
	\end{itemize}
	\begin{center}
	...
	\end{center}
	\begin{itemize}
		\item Where the projection is defined as:
	\end{itemize}
	\begin{center}
...
\frame{
	\frametitle{Intuitions for why QMIX fails}
	\begin{center}
		$\qmixProj Q := \argmin_{q \in \qmixSpace} \sum_{\mathbf{u} \in \mathbf{U}} 
(Q(s,\mathbf{u}) - q(s,\mathbf{u}))^2$
	\end{center}
	\begin{itemize}
		\item We are weighting the error for \textbf{all} joint-actions equally in this idealised setting.
	\end{itemize}
	\begin{center}
		\begin{tabular}{ | c | c | c |}
			\hline
	...
	\end{center}
	\begin{itemize}
		\item Hence we fail on this simple matrix game.
	\end{itemize}
...
\frame{
	...
	\begin{itemize}
		\item What happens if we \textbf{only} considered the error of the optimal joint-action?
	\end{itemize}
	\begin{center}
		\begin{tabular}{ | c | c | c |}
			\hline
	...
	\end{center}
	\begin{itemize}
		\item For a single joint-action, the representational limitations of QMIX have no affect.
		\item Ideally, we also want to learn $Q$-values for the other joint-actions that are below the optimal action's $Q$.
	\end{itemize}
...
\frame{
	...
	\begin{itemize}
		\item Based on that intuition, we can introduce a weighting function into our objective:
	\end{itemize}
	\begin{center}
...
\frame{
	...
	\begin{itemize}
		\item Idealised Central Weighting:
	\end{itemize}
	\begin{multicols}{2}
		\begin{equation*}
		    w(s,\mathbf{u}) = 
	...
	\columnbreak
	\begin{itemize}
		\item Not a practical weighting
		\item Needs approximation in experiments
	\end{itemize}
	\end{multicols}

	\begin{itemize}
		\item Optimistic Weighting:
	\end{itemize}
	\begin{multicols}{2}
		\begin{equation*}
			w(s,\mathbf{u}) = 
	...
	\columnbreak
	\begin{itemize}
		\item Easy to use in experiments
		\item Less obvious it works
	\end{itemize}
	\end{multicols}
	Have theoretical results for both providing a firm foundation.
}

\frame{
	\frametitle{Weighted QMIX}
	There are 3 components to Weighted QMIX:\\
	\begin{itemize}
		...
		\item Weighted QMIX loss.
			\begin{itemize}
				...
				\item Ensures we recover the correct argmax action and correct maximum $Q$-value.
			\end{itemize}
...
\frame{
	...
	\begin{itemize}
		...
		\item Our analysis provides a firm theoretical foundation to build on top off.
...
\frame{
	...
	\begin{itemize}
		\item \bld{TODO: Add description of reward structure}
	\end{itemize}
	\begin{center}
...
\frame{
	...
	\begin{itemize}
		\item QMIX can be particularly brittle in the presence of significant exploration.
	\end{itemize} 
	\begin{center}
...
\frame{
	...
	\begin{itemize}
		\item Weighted QMIX introduces additional complexity to the method:
			\begin{itemize}
				\item Additional model we must now think about
				\item Weighting function into our loss
			\end{itemize}
		\item This extra complexity can lead to \textit{regressions} in performance compared to QMIX.
	\end{itemize} 
	\begin{center}
...
\begin{itemize}
	...
	\vspace{4mm}
	\item \bld{TODO: update with WQMIX}
\end{itemize}
}
